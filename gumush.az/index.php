...
<html>
<head>
	<link rel="alternate" hreflang="az" href="http://gumush.az/" />
	<meta http-equiv="content-language" content="az" />
	<meta name="keywords" content="qadin gumusleri,kisi gumusleri,usaq gumusleri,gümüş,qadın gümüşləri,gumus satisi,bakida gumus satisi,qadın və kişi gümüşləri,online gumus satisi" />
	...
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
   	...
   	<meta property="og:url" content="http://gumush.az" />
...
<body>
	...
	<div class="container">
		...
		<!-- SLIDER START -->
			<div class="row main_divs_space">
				<script type="text/javascript">
					angular.module("slider_app", [])
					.directive("sliderQiymet", function(){
						return{
							restrict: 'E',
							template: ''
						}
				...
				<div class="slider_main xol-xs-12" ng-app="slider_app">
						<div class="slider_inner_left col-xs-12 col-sm-8 col-sm-push-2 col-md-7 col-md-push-0">
							<img class="slider_img" title="gumush.az gumus" />
						</div>
						<div class="slider_inner_right col-xs-12 col-sm-12 col-md-5">
							<h5 class="slider_h5"></h5>
							<p class="slider_p"></p>
							<slider-qiymet class="slider_qiymet"></slider-qiymet>
							<a class="slider_a_btn">
								<button class="btn btn-success">İndi al</button>
							</a>
						</div>
						<div class="my_slider_pager col-xs-12">
						</div>
						<script type="text/javascript">
								var current_p = 0;
								var cem = [];
								$.ajax({
								    type: 'POST',
								    url: 'bakuweb/farajov/slidere-foto-elave-et/slider_fotolari.json',
								    data: {get_param: 'value'},
								    dataType: 'json',
								    success: function (data){
								        slider_fotolari = data;
								        for(a=0;a<slider_fotolari.length;a++){
								        	cem.push({basliq:slider_fotolari[a].basliq,aciqlama:slider_fotolari[a].aciqlama,qiymet:slider_fotolari[a].qiymet,link:slider_fotolari[a].link,img_alt:slider_fotolari[a].img_alt,a_title:slider_fotolari[a].a_title,img_name:"slider/"+slider_fotolari[a].img_name});
								        }
								        $(".slider_img").attr({
											src:cem[current_p].img_name,
											alt:cem[current_p].img_alt
										});
										$(".slider_h5").html(cem[current_p].basliq);
										$(".slider_p").html(cem[current_p].aciqlama);
										$(".slider_qiymet").html(cem[current_p].qiymet+" AZN");
										$(".slider_a_btn").attr({
											href:cem[current_p].link,
											title:cem[current_p].a_title
										});
										var stop_right_side_animation = false;

										function right_side_animation(){
												if(stop_right_side_animation){
													return;
												}else{
													setTimeout(function(){
														$(".slider_inner_right").css("transition","0.7s");
														$(".slider_inner_right").css("left","670px");
													},3200);
													setTimeout(function(){
														$(".slider_inner_right").css("transition","1.6s");
														$(".slider_inner_right").css("left","0px");
													},4050);
												}
										}
										...
										function my_slider(){
											...
											var auto_change_btn_clr = parseInt(current_p + 1);
											$(".my_slider_pager button:nth-child("+auto_change_btn_clr+")").css("background-color","#ED5258");
											$(".my_slider_pager button:nth-child("+auto_change_btn_clr+")").siblings().css("background-color","#3f3f45");
											
										}
										my_slider();
										function slider_btns(cem){
											k = cem.length;
											for(a=0;a<k;a++){
												document.getElementsByClassName("my_slider_pager")[0].innerHTML += "<button class='slider_buttonlari' id="+a+" ></button>";
											}
										}
										var slider_change_interval = setInterval(function (){
											if(current_p<cem.length-1){
												current_p++;
											}else{
												current_p=0;
											}
										...
										var ignore_more_click_to_change_btns = true;
										$(".slider_buttonlari").on("click",function(){
											id = event.target.id;
											setTimeout(function(){
												ignore_more_click_to_change_btns = true;
											},4500);
											if(ignore_more_click_to_change_btns==true){
												current_p = id;
												setTimeout(function(){
													$("#"+id).css("background-color","#ED5258");
													$("#"+id).siblings().css("background-color","#3f3f45");
												},500);
												clearInterval(slider_change_interval);
												stop_right_side_animation = true;
												setTimeout(function(){
													$(".slider_inner_right").css("transition","0.5s");
													$(".slider_inner_right").css("left","670px");
													$(".slider_inner_right").css("transition","1s");
													$(".slider_inner_right").css("left","0px");
												},1000);
												setTimeout(function(){
													stop_right_side_animation = false;
												},2500);
												slider_change_interval = setInterval(function (){
													if(current_p<cem.length-1){
														current_p++;
													}else{
														current_p=0;
											...
											ignore_more_click_to_change_btns = false;
										});
								    }
								});
							</script>
		...
		<!-- UNDER SLIDER START -->
			<div class="row main_divs_space">
				<div class="under_slider_main">
					<div class="selection3 col-xs-12"> 
						<div class="firstImg col-md-4 col-sm-4 col-xs-12">
							<h2>Qolbaq</h2>
							<a href="qolbaq" title="indi al"><button type="button" class="btn btn-default">İndi Al!</button>
							</a>
						</div>
						<div class="secondImg col-md-4 col-sm-4 col-xs-12">
							<h2>Sırğa</h2>
							<a href="sirga" title="indi al"><button type="button" class="btn btn-default">İndi Al!</button>
							</a>
						</div>
						<div class="thirdImg col-md-4 col-sm-4 col-xs-12">
							<h2>Boyunbağı</h2>
							<a href="boyunbagi" title="indi al"><button type="button" class="btn btn-default">İndi Al!</button>
							</a>
						</div>
					</div>
				</div>
			...
			<div class="row main_divs_space">
					<div class="sellList col-xs-12">
						<ul class="nav nav-tabs">
							<li id="tab_yeniler_satilanlar_on_iyirmi" class="active"><a data-toggle="tab" href="#en_yeniler">Ən yenilər</a></li>
							<li id="tab_yeniler_satilanlar_on_iyirmi"><a data-toggle="tab" href="#en_cox_satilanlar">Ən çox satılanlar</a></li>
							<li id="tab_yeniler_satilanlar_on_iyirmi"><a data-toggle="tab" href="#on_otuz_azn">10 - 30 AZN</a></li>
						</ul>
						<div class="tab-content row">
							<div id="en_yeniler" class="tab-pane fade in active">
								<?php
									//$top12_class vasitesile a taglarina 0,1,2,... class veririk ve asagida ajax ile top top12.json faylindan gelen melumatlari 
									//a tagina attr,img taginada attr ve h2 ile p taglarina html vasitesile basliq ve qiymeti yaziriq.
									$top12_class = 0;
									for($a=0;$a<12;$a++){
										echo "<div class='sellone col-md-3 col-sm-4 col-xs-12 top12-".$top12_class."'>
												...
												<p></p>
												<a class='add_a_btn'>
													<button class='btn btn-success'>Sİfarİş</button>
												</a>
												<a class='more_a_btn'>
								...
								<script type="text/javascript">
									...
									$.ajax({
									    ...
									    success: function (data){
									        top12 = data;
									        for(a=0;a<12;a++){
									        	$(".top12-"+a+" a:nth-child(4)").attr({
									        		title:top12[a].a_title
									        	});
									        	$(".top12-"+a+" a:nth-child(5)").attr({
									        		href:"item/"+top12[a].tmp,
									        		title:top12[a].a_title
									        	});
									        	$(".top12-"+a+" img").attr({
									        		src:"item/"+top12[a].tmp+"/image0.jpg",
									        		alt:top12[a].img_alt
									        	});
									        	$(".top12-"+a+" h2").html(top12[a].basliq);
									        	$(".top12-"+a+" p").html(top12[a].qiymet+" AZN");
									        	$(".top12-"+a+" a:nth-child(4) button").attr({
									        		id:"mehsul"+top12[a].id,
									        		onclick:"sifarish(this.id)"
									        	});
							...
							<div id="en_cox_satilanlar" class="tab-pane fade">
								<?php
									if($conn){
										$select_top_sell = "SELECT id,basliq,qiymet,tmp,a_title,img_alt FROM mehsullar ORDER BY RAND() LIMIT 12";
							...
							<div id="on_otuz_azn" class="tab-pane fade">
								<?php
									if($conn){
										$select_top_sell = "SELECT id,basliq,qiymet,tmp,a_title,img_alt FROM mehsullar WHERE qiymet < 31 and qiymet > 9 ORDER BY RAND() LIMIT 12";
										$netice_top_sell = mysqli_query($conn,$select_top_sell);
										while($row = mysqli_fetch_assoc($netice_top_sell)){
											echo "<div class='sellone col-md-3 col-sm-4 col-xs-12'>
												<div class='imgBox col-xs-12'>
